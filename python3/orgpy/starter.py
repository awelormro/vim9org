...
def org_globals_start():
    ...
    vim.command("setlocal foldexpr=py3eval('fold.fold_cache_py3()')")
    ...
    vim.command('command! -buffer OrgLinkNext py3 links.links_go_next()')
    vim.command('command! -buffer OrgLinkPrev py3 links.links_go_prev()')
    vim.command('command! -buffer OrgLinkEnter py3 links.links_enter()')
